...
路径页用小行星展示当前目标对应的待学依赖子图，真实前置关系以单向箭头连接，独立分支并列、共同依赖汇合，并突出推荐下一步，点击节点打开学习详情，查看前置内容、预计时间与配套练习；不在路径界面展示掌握度、置信度等指标。通过“调整目标”修改目标与每日时间，通过“历史路线”查看只读回放。驾驶舱的路线预览使用相同的路径数据，保存目标、重新规划和学习记录变化后同步更新。
