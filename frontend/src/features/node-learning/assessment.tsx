  ...
  return (
    ...
    >
      ...
      <p className='text-xs text-slate-500'>
        ...
        {t('· 预计')}
        {task.minutes}
        {t('分钟 ·')} {task.kind === 'quiz' ? t('客观测验') : t('主观作答')}
      </p>
      {task.kind === 'quiz' ? (
        ...
        <Textarea
          ...
          placeholder={t('写出你的理解、推理过程和适用条件')}
        />
      )}
      <div className='flex gap-2'>
        ...
        >
          {record.isPending ? t('正在评价…') : t('提交作答')}
        </Button>
        <Button
          size='sm'
          type='button'
      ...
      {hint && (
        <p className='text-xs leading-5 text-sky-300'>
          {t('先回顾当前节点的概念和适用条件，再对照题目说明理由。参考资料：')}
          {task.source}
        </p>
      )}
...
export function NodeAssessment({ nodeId }: { nodeId: string }) {
  ...
  return (
    ...
    >
      ...
      {query.isPending ? (
        <p role='status'>{t('正在加载评价任务…')}</p>
      ) : query.isError ? (
        <p role='alert'>{t('评价任务加载失败，请稍后重试。')}</p>
      ) : (
        data && (
          <>
      ...
      <h3 className='text-sm font-semibold'>{t('最近学习证据')}</h3>
      {events.data
