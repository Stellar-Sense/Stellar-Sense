...
function GoalEditor({ goal, nodes }: { goal: Goal; nodes: GraphNode[] }) {
    ...
    <form
      ...
      onSubmit={(event) => {
        ...
        update.mutate(
          ...
          { onSuccess: () => toast.success('目标与学习时间已保存') }
        )
      }}
    >
  ...
  return (
    ...
    >
      <div className='flex flex-wrap items-start justify-between gap-3'>
        <div>
          <p className='mb-1 text-xs text-slate-400'>
            {entry.domain} · {entry.isGoal ? '目标节点' : '先修补学'}
            {entry.isReview ? ' · 复习巩固' : ''}
          </p>
        ...
        >
          ...
          {entry.status === 'ready' ? '可立即学习' : '等待前置达标'}
        </span>
      </div>
      <div className='my-3 flex flex-wrap gap-3 text-xs text-slate-300'>
        <span>掌握度 {entry.mastery}%</span>
        <span>置信度 {entry.confidence}%</span>
        <span>{difficultyLabels[entry.difficulty]}</span>
      ...
      <p className='mt-3 text-xs text-violet-300'>
        {entry.sessions
          .map((session) => `第 ${session.day} 天 · ${session.minutes} 分钟`)
          .join(' / ')}
      </p>
      {entry.taskTitle && (
        <p className='mt-2 text-xs text-slate-300'>
          评价任务：{entry.taskTitle}
        </p>
      )}
      {!readOnly && (
          ...
          <Button
            ...
            onClick={() =>
              record.mutate(
                ...
                {
                  onSuccess: () =>
                    toast.success('已记录跳过偏好，必要先修仍会保留'),
                }
              )
            }
          >
...
export function PathPlanning() {
  ...
  return (
    <LearningPage
      title='学习路径'
      description='从你的目标出发，依据先修关系与学习证据安排补学、练习和复习。目标或证据变化后，路径会重新计算。'
      actions={
        <Button
          variant='outline'
            ...
            <div className='grid gap-3 sm:grid-cols-3'>
              {[
                ...
                },
                {
                  icon: Clock3,
                  label: '按当前预算预计',
                  value: `${data.totalMinutes} 分钟 · ${data.estimatedDays} 天`,
                },
              ...
              <section className={`${panelClass} space-y-4`}>
                <div className='flex items-center justify-between gap-3'>
                  <h2 className='font-semibold'>
                    {version ? `历史路径 · 第 ${version} 版` : '目标与补学路线'}
                  </h2>
                  {version && (
                    <Button
                      size='sm'
                ...
                {version ? (
                  replay.isPending ? (
                    <p role='status'>正在回放历史路径…</p>
                  ) : replay.isError ? (
                    <p role='alert'>回放失败，请重试或返回当前路径。</p>
                  ) : (
                    replay.data && (
                      <>
                        <div className='rounded-xl bg-violet-500/10 p-3 text-xs leading-6 text-violet-200'>
                          {replay.data.verified
                            ? '冻结输入重新计算一致'
                            : '当前规则版本无法验证此记录'}{' '}
                          · 图谱版本 {replay.data.graphRevision}
                          <br />
                          {replay.data.eventIds.length} 条学习事件 ·{' '}
                          {replay.data.evaluationIds.length} 条评价 · 每天{' '}
                          {replay.data.goal.dailyMinutes} 分钟
                          <br />
